                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    // Get the two start and end points of the stroke in 3D
    Point3 S;
    Point3 E;
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[0], &S);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[stroke2d.size() - 1], &E);
    
    Vector3 EtoS = S-E;
    
    Vector3 PlaneNormal = Vector3(0, 1, 0).Cross(EtoS).ToUnit();

    std::vector<Point3> vertexOnPlane3D;
    ...
    for (int i = 0; i < stroke2d.size(); i++)
    {
        float iTime;
        Point3 strokeOnProj;
        Point3 strokeIn3DScreen = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, stroke2d[i]);
        Ray eyeThroughStroke = Ray(eye, strokeIn3DScreen - eye);

        eyeThroughStroke.IntersectPlane(S, PlaneNormal, &iTime, &strokeOnProj);
        vertexOnPlane3D.push_back(strokeOnProj);
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        Point3 ClosestPointOnPlane = P.ClosestPointOnPlane(S, PlaneNormal);
        float h = hfunc(PlaneNormal, vertexOnPlane3D, ClosestPointOnPlane);

        if (h != 0)
        {
            float y = P.y();
            float d = (P - ClosestPointOnPlane).Length();

            float dw = 1.0f - ((d / 5.0f) * (d / 5.0f));
