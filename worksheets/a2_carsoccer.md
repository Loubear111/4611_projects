...
// Assuming 1 unit per ms for velocity
...
bool sphereIntersection(Sphere s1, Sphere s2) {
    ...
    Vector3 vTos2 = s1.position - s2.position;

    // If the vector pointing from the center of s1 to center of s2
    // is shorter in length then both the radii summed, then the circles
    // are colliding
    if(vTos2.Length() <= s1.radius + s2.radius)
    {
	return true
    }
    return false
